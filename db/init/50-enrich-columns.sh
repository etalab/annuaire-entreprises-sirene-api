...
psql -U $POSTGRES_USER -d $POSTGRES_DB -c "UPDATE siren SET tsv = setweight(to_tsvector(coalesce(denominationunitelegale,'')), 'A') || setweight(to_tsvector(coalesce(sigle,'')), 'A') || setweight(to_tsvector(coalesce(nomunitelegale,'')), 'B') || setweight(to_tsvector(coalesce(prenom1unitelegale,'')), 'C');"
